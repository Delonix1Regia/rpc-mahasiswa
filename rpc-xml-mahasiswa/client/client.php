...
{	private $host="localhost";	
	...
	*/
	...
	{	$data = xmlrpc_encode_request("method",array("nim"=>$data['nim'],"nama"=>$data['nama'],"alamat"=>$data['alamat'],"hp"=>$data['hp'],"aksi"=>$data['aksi']));
		$context = stream_context_create(array('http' => array(
		    'method' => "POST",
		    'header' => "Content-Type:text/xml;charset=UTF-8",
		    'content' => $data
	...
	{	// query ke lokal database client
		...
		{	// query insert data ke lokal database client
			$query = $this->conn->prepare("insert into mahasiswa (nim,nama,alamat,hp) values (?,?,?,?)");	
			$query->execute(array($r['nim'],$r['nama'],$r['alamat'],$r['hp']));				
			$query->closeCursor();	
		}		 
...
$url = 'http://192.168.56.2/rpc-xml-mahasiswa2/rpc-xml-mahasiswa/server/server.php';
